...
Status s2d_n(const char* buffer, uintptr_t len, double* result)
{
	...
	int		 m10digits = 0;
	int		 e10digits = 0;
	uintptr_t		 dotIndex  = len;
	uintptr_t		 eIndex	   = len;
	uint64_t m10	   = 0;
	int32_t	 e10	   = 0;
	bool	 signedM   = false;
	bool	 signedE   = false;
	...
	e10 -= dotIndex < eIndex ? static_cast<int32_t>(eIndex - dotIndex - 1) : 0;
	if(m10 == 0)
	{
	...
	uint32_t ieee_e2 = std::max<uint32_t>(0, e2 + DOUBLE_EXPONENT_BIAS + floor_log2(m2));
