...
{
	uint64_t b00Hi;
	core::umul(aLo, bLo, b00Hi);
	uint64_t		b01Hi;
	const uint64_t	b01Lo = core::umul(aLo, bHi, b01Hi);
	uint64_t		b10Hi;
	const uint64_t	b10Lo = core::umul(aHi, bLo, b10Hi);
	uint64_t		b11Hi;
	const uint64_t	b11Lo = core::umul(aHi, bHi, b11Hi);
...
static inline uint32_t mulShift_mod1e9(const uint64_t m, const std::array<uint64_t, 3>& mul, const int32_t j)
{
	uint64_t high0;											// 64
	core::umul(m, mul[0], high0);							// 0
	uint64_t		high1;									// 128
	const uint64_t	low1 = core::umul(m, mul[1], high1);	// 64
	uint64_t		high2;									// 192
	const uint64_t	low2 = core::umul(m, mul[2], high2);	// 128
		...
		for(; i < blocks; ++i)
		{
			...
			else
			{
				...
				else
				{
					...
					const bool	  trailingZeros = requiredTwos <= 0 || (requiredTwos < 60 && multipleOfPowerOf2(m2, (uint32_t) requiredTwos));
...
uintptr_t d2exp_buffered_n(double d, uint32_t precision, char* result)
{
	...
	else
	{
		...
		bool		  trailingZeros = requiredTwos <= 0 || (requiredTwos < 60 && multipleOfPowerOf2(m2, (uint32_t) requiredTwos));
		if(rexp < 0)
		{
